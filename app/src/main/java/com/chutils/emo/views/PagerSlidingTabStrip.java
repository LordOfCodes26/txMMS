...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            switch (tabType){
                case 1:{
                    tabView = LayoutInflater.from(getContext()).inflate(R.layout.psts_imagetab, this, false);
                    View selectionView = tabView.findViewById(R.id.psts_tab_selection);
                    selectionView.setBackgroundResource(R.drawable.ch350_psts_tab_selection);
                    break;
                }
                case 2:{
                ...
                case 3:{
                    tabView = LayoutInflater.from(getContext()).inflate(R.layout.psts_sentence_tab, this, false);
                    break;
                }
                default:{
                    tabView = LayoutInflater.from(getContext()).inflate(R.layout.psts_tab, this, false);
            ...
            if (mPager.getAdapter() instanceof EmoViewPageAdapter){
                EmoViewPageAdapter emogiViewPageAdapter = (EmoViewPageAdapter) mPager.getAdapter();

                Bitmap bitmap = null;
                try{
                    EmoModel emo =  emogiViewPageAdapter.mVisTabData.get(i);
                    bitmap = emo.getThumbBitmap();
                    ...
                    tabView.setOnClickListener(new OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            if (mPager.getCurrentItem() != pos) {
                                ...
                                if (Math.abs(mPager.getCurrentItem() - pos) < 2) {
                                ...
                                mTabReselectedListener.onTabReselected(pos);
                            }
                        }
                    });
                }
    ...
    private void addTab(final int position, Bitmap title, View tabView) {
        ImageView textView = (ImageView) tabView.findViewById(R.id.psts_tab_title);
        if (textView != null) {
            if (title != null) textView.setImageBitmap(title);
        }

        tabView.setFocusable(true);

        float width = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 35f, getResources().getDisplayMetrics());
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams((int) width, LayoutParams.MATCH_PARENT);
        ...
        tabView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (mPager.getCurrentItem() != position) {
                    ...
                    if (Math.abs(mPager.getCurrentItem() - position) < 2) {
    ...
    private void scrollToChild(int position, int offset) {
        switch (tabType){
            case 1:{
                scrollToChildEmogi(position,offset);
                 break;
            }
            case 2:
            case 3:
            default:{
                scrollToChildNormal(position,offset);
                break;
            }
        }
    }
    private void scrollToChildEmogi(int position, int offset) {
        if (mTabCount == 0) {
            return;
        }
        ...
        int childLeft = mTabsContainer.getChildAt(position).getLeft();
        int childRight = mTabsContainer.getChildAt(position).getRight();
        boolean stickLeft = true;
        if(getScrollX() < childLeft){
            if (getScrollX() + getWidth() > childRight){
                return;
            }else{
                stickLeft = false;
            }
        ...
        int newScrollX;
        if (stickLeft){
            newScrollX = childLeft + offset;
        }else{
            newScrollX = childRight - getWidth();
...
//        }
    ...
    private void scrollToChildNormal(int position, int offset) {
        if (mTabCount == 0) {
            return;
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (isPaddingMiddle || mPaddingLeft > 0 || mPaddingRight > 0) {
            ...
            if (isPaddingMiddle) {
                ...
                // Account for manually setItem padding for offsetting tab start and end positions.
    ...
    private void unSelect(View tab) {
        if (tab != null) {
            switch (tabType){
                case 1:{
                    View  tab_view =  tab.findViewById(R.id.psts_tab_root);
                    if (tab_view != null){
                        View view = tab_view.findViewById(R.id.psts_tab_selection);
                ...
                case 3:
                default:{
                    View  tab_view =  tab.findViewById(R.id.psts_tab_title);
                    if (tab_view != null)
                        tab_view.setSelected(false);
                    break;
                }
            }
    ...
    public static Drawable getSelectionDrawable(int colorNormal, int colorSelected){
        StateListDrawable drawable = new StateListDrawable();
        drawable.addState(new int[] { -android.R.attr.state_enabled }, createInnerStrokesDrawable(colorNormal, 1));
        drawable.addState(new int[] { android.R.attr.state_selected }, createInnerStrokesDrawable(colorSelected, 1));
        drawable.addState(new int[] { }, createInnerStrokesDrawable(colorNormal, 1));
        return drawable;
    }

    public static Drawable createInnerStrokesDrawable(final int color, float strokeWidth) {
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            switch (tabType){
                ...
                case 2:{
                    v.setPadding(mTabPadding, v.getPaddingTop(), mTabPadding, v.getPaddingBottom());
                    TextView tvTabTitle = (TextView) v.findViewById(R.id.psts_tab_title);
                    if (tvTabTitle != null) {
                        tvTabTitle.setTextColor(mTabTextColor);
                    }
                    break;
                }
                case 3:{
                    View tab_view =  v.findViewById(R.id.psts_tab_title);
                    tab_view.setPadding(mTabPadding, v.getPaddingTop(), mTabPadding, v.getPaddingBottom());
                    TextView tvTabTitle = (TextView) v.findViewById(R.id.psts_tab_title);
                    tvTabTitle.setBackgroundResource(mTabBackgroundResId);
                    if (tvTabTitle != null) {
                        tvTabTitle.setTextColor(mTabTextColor);
                    }
                    break;
                ...
                default:{
                    Object  tab_view =  v.findViewById(R.id.psts_tab_title);
                    v.setBackgroundResource(mTabBackgroundResId);
                    v.setPadding(mTabPadding, v.getPaddingTop(), mTabPadding, v.getPaddingBottom());
                    TextView tvTabTitle = (TextView) tab_view;
                    if (tvTabTitle != null) {
                        tvTabTitle.setTextColor(mTabTextColor);
                        if (isTabTextAllCaps) {
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                                tvTabTitle.setAllCaps(true);
                            }
                            else {
